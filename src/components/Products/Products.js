...
function Products(props) {
  ...
  let displayedItems = props.searchResults.map((item) => (
    <div key={item.id} className="product-card">
      ...
      <button
        onClick={() => {
          // props.cart.some((e) => e.id === item.id)
          //   ? props.removeFromCart(item)
          //   : props.addToCart(item);
          if (props.cart.some((e) => e.id === item.id)) {
            console.log(`Removing item with id ${item.id} from cart`);
            props.removeFromCart(item); // Item already in cart, remove it
          } else {
            console.log(`Adding item with id ${item.id} to cart`);
            props.addToCart(item); // Item not in cart, add it
          }
